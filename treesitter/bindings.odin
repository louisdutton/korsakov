...
Language :: distinct rawptr // Opaque pointer
Parser :: distinct rawptr // Opaque pointer
Tree :: distinct rawptr // Opaque pointer
Query :: distinct rawptr // Opaque pointer
Query_Cursor :: distinct rawptr // Opaque pointer
Lookahead_Iterator :: distinct rawptr // Opaque pointer
...
@(link_prefix = "ts_")
foreign ts {
  parser_new :: proc() -> Parser --- // Create a new parser.
  parser_delete :: proc(self: Parser) --- // Delete the parser, freeing all of the memory that it used.
  parser_language :: proc(self: Parser) -> Language --- // Get the parser's current language.
  ...
  parser_set_included_ranges :: proc(self: Parser, ranges: [^]Range, count: u32) -> bool ---

  // Get the ranges of text that the parser will include when parsing.
  parser_included_ranges :: proc(self: Parser, count: ^u32) -> [^]Range ---
  ...
  parser_parse_string_encoding :: proc(self: Parser, old_tree: Tree, string: cstring, length: u32, encoding: Input_Encoding) -> Tree ---
  ...
  parser_set_timeout_micros :: proc(self: Parser, timeout_micros: u64) ---

  // Get the duration in microseconds that parsing is allowed to take.
  parser_timeout_micros :: proc(self: Parser) -> u64 ---
  ...
  parser_print_dot_graphs :: proc(self: Parser, fd: i32) ---
  ...
  tree_included_ranges :: proc(self: Tree, length: ^u32) -> [^]Range ---
  ...
  tree_get_changed_ranges :: proc(old_tree: Tree, new_tree: Tree, length: ^u32) -> [^]Range ---

  // Write a DOT graph describing the syntax tree to the given file.
  tree_print_dot_graph :: proc(self: Tree, file_descriptor: i32) ---
  ...
  query_predicates_for_pattern :: proc(self: Query, pattern_index: u32, step_count: ^u32) -> [^]Query_Predicate_Step ---
  ...
  query_disable_capture :: proc(self: Query, name: cstring, length: u32) ---
  ...
  language_symbol_for_name :: proc(self: Language, string: cstring, length: u32, is_named: bool) -> Symbol ---
  ...
  language_field_id_for_name :: proc(self: Language, name: cstring, name_length: u32) -> Field_Id ---
...
foreign ts {
  ...
  wasm_store_load_language :: proc(self: ^Wasm_Store, name: cstring, wasm: cstring, wasm_len: u32, error: ^Wasm_Error) -> Language ---
...
foreign ts {
  ...
  set_allocator :: proc(new_malloc: (proc(_: uint) -> rawptr), new_calloc: (proc(_: uint, _: uint) -> rawptr), new_realloc: (proc(_: rawptr, _: uint) -> rawptr), new_free: (proc(_: rawptr))) ---
}
