...
compat_allocator_init :: proc(
  rra: ^Compat_Allocator,
...
compat_allocator :: proc(rra: ^Compat_Allocator) -> mem.Allocator {
  return mem.Allocator{data = rra, procedure = compat_allocator_proc}
}
  ...
  tree_sitter_log :: proc "c" (
    ...
    if len(str) == len(buf) {
      context.logger.procedure(
        context.logger.data,
        .Error,
        "tree-sitter log exceeded 1kib and could not be fully printed",
        context.logger.options,
      )
    }
  }
